...
class Book(db.Model):
    ...
    categoryid = db.Column(db.Integer(8), db.ForeignKey('categories.id', ondelete='CASCADE'))
            

class Bookshelf(db.Model): 
    __tablename__ = 'bookshelf'   #테이블 이름 : bookshelf
    userid = db.Column(db.Integer(8), db.ForeignKey('user.id', ondelete='CASCADE'))
    bookid = db.Column(db.Integer(8), db.ForeignKey('book.id', ondelete='CASCADE'))